    ...
    return (
        ...
        >
            {(state) => (
                ...
                >
                    {props.type === "icon" && !stacked && <span style={{ marginRight: '20px', border: '1px solid #DDD', borderRadius: '6px', padding: '4px', display: 'inline-flex' }}><props.icon className="size-5 text-fg-quaternary" /></span>}
                    {props.type === "icon" && stacked && <span style={{ marginRight: '20px' }}><FeaturedIcon color="gray" size="md" theme="modern" icon={props.icon} /></span>}
