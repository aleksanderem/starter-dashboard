...
export const CommandInput = ({ placeholder, shortcutKeys, tooltip, className, ...props }: CommandInputProps) => {
    return (
        ...
        >
            <div className="pointer-events-none absolute" style={{ marginLeft: '7px' }}>
                <SearchLg className="size-5 text-fg-quaternary" />
            </div>

            <AriaInput
                placeholder={placeholder}
                className="m-0 w-full bg-transparent text-md text-primary ring-0 outline-hidden placeholder:text-placeholder autofill:rounded-lg autofill:text-primary"
                style={{ padding: '10px 30px', borderRadius: '8px', border: '1px solid #DDD' }}
                {...props}
            />
